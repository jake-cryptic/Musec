var tiles = {
	...
	desktopNotify: function(t,b,i) {
		...
		if (!("Notification" in window)) {
		...
		} else if (Notification.permission === "granted" && tiles.m != true) {
			...
			Notification.requestPermission(function (permission) {
				if (permission === "granted" && tiles.m != true) {
					tiles.notif = new Notification(t,options);
					setTimeout(tiles.notif.close.bind(tiles.notif),7500);
				}
			});
	...
	nextSong: function(){
		...
		tiles.folder.html("Loading...");
		
		
		AudioHelper.stopPreviousPlayback();
		AudioHelper.getAudioFile(loc);
	...
	updateMediaInfo:function() {
		if (AudioHelper.MSC.isPlaying) {
			if (AudioHelper.BFR == null || typeof(AudioHelper.BFR.duration) == "undefined" || isNaN(AudioHelper.BFR.duration)) {
				...
				var cPos = (AudioHelper.CTX.currentTime - AudioHelper.MSC.contextCreate) / AudioHelper.SRC.playbackRate.value;
				
				tiles.PlayBackSlider.value = (cPos * (100 / AudioHelper.BFR.duration));
				
				var currentMinutes = Math.floor(cPos / 60);
				var currentSeconds = Math.floor(cPos - currentMinutes * 60);
				var totalMinutes = Math.floor(AudioHelper.BFR.duration / 60);
				var totalSeconds = Math.floor(AudioHelper.BFR.duration - totalMinutes * 60);
	...
	songSeek:function(){
		AudioHelper.seekPlayback(AudioHelper.BFR.duration * (tiles.PlayBackSlider.value / 100));
	...
	queueView:function(){
		...
		if (tiles.songQueue.length == 0) {
			...
			for(var i = 0;i < ((tiles.songQueue.length-1)/2);i++) {
				...
				sB = "<tr><td class='clickable' onclick='tiles.goToSong(" + i + ")'>" + stat + "</td><td class='clickable' onclick='tiles.goToSong(" + i + ")'>" + tiles.songQueue[(i*2)+1] + "</td><td>" + queueCtrls + "</td></tr>";
...
$(document).ready(function(){
	...
	if (tiles.m === true) {
		...
		if((navigator.userAgent.match(/iPhone/i)) || (navigator.userAgent.match(/iPod/i))) {
			tiles.sAlert("Welcome","../Musec!3.jpg",500);
	...
	$(window).resize(function(){
		...
		AudioHelper.MSC.AV_W = window.innerWidth*2;
		AudioHelper.MSC.AV_H = (window.innerHeight*2)-200;
