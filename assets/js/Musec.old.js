...
var tiles = {
	...
	loadIndex: function(cb) {
		$.get(window.defaultPath + "resources/music_index.json", function(data) {
			tiles.songIndex = data;
			cb(1);
		}).fail(function() {
			cb(0);
		});
	...
	//loadSongs
	generateAlbumPage: function(folder, method) {
		if (method == 1) {
		...
		tiles.load("t=s&d=" + btoa("/" + folderName), function() {
	...
	showFolder: function(folderData) {
		...
		if (folderData.data.length == 0) {
			...
			$(".tile_longclick").each(function(x) {
				$("#tile_id_" + x + "_bg").contextmenu(function(evn) {
					evn.preventDefault();
					tiles.showTileMenu(x);
				});
				$("#tile_id_" + x + "_bg").longclick(250, function() {
					tiles.showTileMenu(x);
				});
				$("#tile_id_" + x + "_bg").click(function() {
					tiles.loadSongs(x, 1);
				});
			});
		}
	...
	showSongs: function(folderData, responseType) {
		...
		if (responseType == 1) {
			if (folderData.data.length == 0) {
				$sf.html("<h2>Couldn't find any music in this directory</h2>");
				return;
			}
			...
			tiles.folder.fadeIn(300, function() {
				tiles.folder.animate({
					width: eW,
					opacity: 1
				}, 500);
			});
			$("#search_container").fadeOut(300, function() {
	...
	nextSong: function(sP) {
		...
		tiles.AudioElement.addEventListener('loadeddata', function() {
			...
			tiles.PlayBackSlider.addEventListener("mousedown", function() {
				tiles.allowThumbMove = false;
			}, false);
			tiles.PlayBackSlider.addEventListener("touchstart", function() {
				tiles.allowThumbMove = false;
			}, false);
			tiles.PlayBackSlider.addEventListener("mouseup", function() {
				tiles.allowThumbMove = true;
			}, false);
			tiles.PlayBackSlider.addEventListener("touchend", function() {
				tiles.allowThumbMove = true;
			}, false);
		...
		tiles.AudioElement.addEventListener("play", function() {
			tiles.changeMediaState(true);
		}, false);
		tiles.AudioElement.addEventListener("pause", function() {
	...
	songLoadProgress: function() {
		...
		if (isNaN(percentRounded)) {
			tiles.folder.css({
				background: "linear-gradient(to right, " + coloursProgess[1] + " 0%, " + coloursProgess[0]
			});
		} else {
			tiles.folder.css({
				background: "linear-gradient(to right, " + coloursProgess[1] + " " + percentRounded + "%, " + coloursProgess[0]
			});
		}
	},
	...
	showTileMenu: function(song_id) {
		...
		if (typeof(tiles.cWt) == "undefined") {
			...
			tiles.cWt.contextmenu(function(evn) {
				evn.preventDefault();
				tiles.showTileMenu(song_id);
			});
			tiles.cWt.longclick(300, function() {
				tiles.showTileMenu(song_id);
			});
		} else {
	...
	alterQueue: function(whatDo, songID) {
		if (whatDo == "add") {
			...
			var initalIndex = ((songID[0] - 1) * 2),
				newIndex = ((songID[1] - 1) * 2);
	...
	reloadQueueView: function() {
		...
		$("#queueFolder").delay(tiles.queueDelay).animate({
			opacity: 1
		}, tiles.queueDelay);
	},
	queueView: function() {
	...
	reloadOfflineView: function() {
		$("#offlineFolder").animate({
			opacity: 0.25
		}, tiles.queueDelay);
		setTimeout(function() {
			tiles.offlineView();
		}, tiles.queueDelay);
		$("#offlineFolder").delay(tiles.queueDelay).animate({
			opacity: 1
		}, tiles.queueDelay);
	},
	offlineView: function() {
		...
		if (MusecOffline.Store == null || typeof(MusecOffline.Store) == "undefined" || !tiles.supportsFS) {
			...
			var songs = [];
			try {
				MusecOffline.Store.getDir("/audio", {
					create: true
				}, function() {
	...
	playOfflineSong: function(osn) {
		console.log(osn);
		MusecOffline.Store.getFile("audio/" + atob(osn), {
			create: false
		}, function(fileEntry) {
	...
	doSearch: function(manualSearch) {
		...
		if (searchTerm == "" || searchTerm.length == 1) {
			...
			if (searchPattern.test(searchTerm)) {
				...
				tiles.load("t=l&s=" + btoa(searchTerm), function() {
					$(".tile_bg").removeClass("blur");
					$("#sAlert").fadeOut(200);
				});
				if ($("#songFolder").is(":visible")) {
	...
	widthCheck: function() {
		if ($(document).width() <= 440) {
			eW = "100%";
		} else if ($(document).width() < 770 && $(document).width() > 440) {
			eW = "64%";
		} else {
			eW = "80%";
		}
	...
	makeOffline: function(url, nm) {
		...
		MusecOffline.Store.getData(url, function(data) {
			...
			MusecOffline.Store.getDir("audio", {
				...
				MusecOffline.Store.write("audio/" + nm, "audio/mp3", data, {
					create: true
				});
...
$(document).ready(function() {
	tiles.load("t=v");
	tiles.loadIndex(function(r) {
		if (r === 0) {
			console.warn("Music Index failed to load");
			// Try to load offline version
			tiles.generateAlbumPage();
		}
	});
	...
	tiles.folder.longclick(250, function() {
		tiles.togglePanel();
	});
	tiles.folder.contextmenu(function(evn) {
		evn.preventDefault();
		tiles.togglePanel();
	});

	tiles.mediaStateTrigger.addEventListener("click", tiles.toggleMediaState, false);
	tiles.searchBox.keyup(function() {
		tiles.preemptSearch();
	});
	tiles.searchBox.keypress(function(k) {
		if (k.which == 13) {
			tiles.doSearch();
		}
	}); // Enter to do search
	tiles.searchDo.click(function() {
		tiles.doSearch();
	});
	...
	tiles.bB.click(function() {
		...
		if (tiles.bB.prop("do") == "refresh") {
			tiles.load("t=f&d=" + btoa("/"), function() {
				$(".tile_bg").removeClass("blur");
			});
	...
	tiles.sB.click(function() {
		...
		if (tiles.folder.is(":visible")) {
			...
			sc.animate({
				...
				sc.fadeIn(300, function() {
					tiles.searchBox.focus();
					tiles.searchBox.click();
				});
			});
		} else {
			eW = tiles.widthCheck();
...
if (/Chrome/.test(navigator.userAgent)) {
	...
	setInterval(function() {
		if (tiles.ext_id == "") {
			tiles.ext_id = prompt("Enter your extension ID", "mjfpmchbcjmomgfbjcfaknigbdjcgjih");
		}
		if (typeof(tiles.AudioElement) == "undefined" || tiles.ext_id == "") {
			return;
		} else {
			// UI Colour
			var iCol = "";
